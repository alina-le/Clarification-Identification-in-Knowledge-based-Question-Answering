...
class ClaquaCorpusMultiTurn:
    ''' class for processing Multi-turn split of CLAQUA corpus: preprocessing of corpus for Transformer tokenization and modeling
    '''
    ...
    def __init__(self):
        ...
        self.turn_sep_tok = None # Transformer special token to separate the different turns in the context (multi-turn context looks like: question_p - answer_p - question_a)
        ...
        self.num_special_tok_positions = 2  # number of special tokens added to separate context and entities: context [CONTEXT_SEP] ent1 [ENTITY_SEP] ent2; turn sep token is not included, because it is already inside context, no need to count it extra later when cutoff truncation ratio calculated
    ...
    def replace_context_seps(self, context_col_name):
        ''' takes string entry from pandas df column; replaces context turn sep tags from corpus with custom separator for turns in context
        multi-turn context looks like: q_p <EOS> a_p <EOS> q_a
        and gets changed into: q_p [CUSTOM_TURN_SEP] a_p [CUSTOM_TURN_SEP] q_a
        '''
        self.df[context_col_name] = self.df[context_col_name].str.replace(r'<EOS>', self.turn_sep_tok, regex=False) 

    def remove_entity_attrbts(self, ent_col_name):
        ''' takes string entry from pandas df column; removes entity attributes from string with entity name, entity text and entity attributes 
        '''
        self.df[ent_col_name] = self.df[ent_col_name].str.replace(r'<S> .* <S>', '', regex=True)  # entity attributes in corpus are enclosed in '<S>' tag

    def set_tokenizer_vars(self, tokenizer, turn_sep_tok, context_sep_tok, entity_sep_tok, num_max_toks):
        ...
        self.turn_sep_tok = turn_sep_tok # Transformer special token to separate the different turns in the context (context in multi-turn = question_p - answer_p - question_a)
    ...
    def encode_text(self, text_item):
        ''' apply Transformer tokenizer to text string, which adds Transformer sep tokens at beginning and end
        in: text
        out: [CLS] text [SEP]
    ...
    def pad_encode_text(self, text_item):
        ''' apply Transformer tokenizer to text string with extended settings: using max length and padding (the cutting to max_length already performed by other functions, but padding needed for shorter sequences)
        :return: tokenizer object with input_ids, token_type_ids and attention_mask
        '''
...
def process_multi_turn_corpus(corpus, tokenizer, turn_sep_tok, context_sep_tok, entity_sep_tok, num_max_toks):
    ...
    corpus.set_tokenizer_vars(tokenizer=tokenizer, turn_sep_tok=turn_sep_tok, context_sep_tok=context_sep_tok, entity_sep_tok=entity_sep_tok, num_max_toks=num_max_toks)
    
    # replace context separator tags with custom separator turn_sep_tok
    logging.info(f'\t Replacing context turn sep tokens by custom separator {turn_sep_tok} ...')
    corpus.replace_context_seps('context')
    ...
    logging.info(f'\t Merging context and truncated entity texts with custom special tokens {context_sep_tok} and {entity_sep_tok}, encode with padding...')
